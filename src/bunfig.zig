    ...
    pub const Parser = struct {
            ...
            if (expr.asArray()) |array_| {
                ...
                while (array.next()) |item| {
                    ...
                    if (!item.data.e_string.isEmpty())
                        preloads.appendAssumeCapacity(try item.data.e_string.toWtf8MayAlloc(allocator));
                }
                ...
                if (!expr.data.e_string.isEmpty()) {
                    ...
                    preloads[0] = try expr.data.e_string.toWtf8MayAlloc(allocator);
        ...
        pub fn parse(this: *Parser, comptime cmd: Command.Tag) !void {
            ...
            if (json.get("define")) |expr| {
                ...
                for (properties) |prop| {
                    if (prop.value.?.data != .e_string) continue;
                    keys[i] = prop.key.?.data.e_string.toWtf8MayAlloc(allocator) catch bun.outOfMemory();
                    values[i] = prop.value.?.data.e_string.toWtf8MayAlloc(allocator) catch bun.outOfMemory();
                    i += 1;
                }
            ...
            if (json.get("origin")) |expr| {
                try this.expectString(expr);
                this.bunfig.origin = try expr.data.e_string.toWtf8MayAlloc(allocator);
            }
            ...
            if (comptime cmd == .TestCommand) {
                if (json.get("test")) |test_| {
                    ...
                    if (test_.get("reporter")) |expr| {
                        ...
                        if (expr.get("junit")) |junit_expr| {
                            ...
                            if (!junit_expr.data.e_string.isEmpty()) {
                                ...
                                this.ctx.test_options.reporter_outfile = try junit_expr.data.e_string.toWtf8MayAlloc(allocator);
                            }
                        }
                    }
                    ...
                    if (test_.get("coverageDir")) |expr| {
                        try this.expectString(expr);
                        this.ctx.test_options.coverage.reports_directory = try expr.data.e_string.toWtf8MayAlloc(allocator);
                    }
            ...
            if (json.get("bundle")) |_bun| {
                if (comptime cmd == .BuildCommand or cmd == .RunCommand or cmd == .AutoCommand or cmd == .BuildCommand) {
                    if (_bun.get("outdir")) |dir| {
                        ...
                        this.bunfig.output_dir = try dir.data.e_string.toWtf8MayAlloc(allocator);
                    }
                }
                ...
                if (comptime cmd == .BuildCommand) {
                    ...
                    if (_bun.get("entryPoints")) |entryPoints| {
                        ...
                        for (items, 0..) |item, i| {
                            try this.expectString(item);
                            names[i] = try item.data.e_string.toWtf8MayAlloc(allocator);
                        }
                    ...
                    if (_bun.get("packages")) |expr| {
                        ...
                        for (properties) |prop| {
                            ...
                            const path = try prop.key.?.data.e_string.toWtf8MayAlloc(allocator);
                ...
                switch (expr.data) {
                    ...
                    .e_array => |array| {
                        ...
                        for (array.items.slice(), 0..) |item, i| {
                            try this.expectString(item);
                            externals[i] = try item.data.e_string.toWtf8MayAlloc(allocator);
                        }
